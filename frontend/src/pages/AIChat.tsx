...
export const AIChat: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-10rem)] md:h-[calc(100vh-8.5rem)] flex flex-col justify-between max-w-4xl mx-auto rounded-2xl bg-[#101623] border border-white/5 overflow-hidden shadow-xl relative">
      
      {/* Header Panel */}
      <div className="px-4 py-3 md:px-6 md:py-4 border-b border-white/5 flex flex-col sm:flex-row gap-3 sm:items-center justify-between bg-[#0c101a]">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="text-xs bg-[#090d16] border border-white/10 text-slate-300 rounded-xl px-2.5 py-1.5 outline-none max-w-[180px] focus:border-blue-500/50"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
          ...
          >
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs shrink-0 shadow-md ${
              msg.role === 'user'
                ...
                : 'bg-[#0c101a] text-blue-400 border border-white/5'
            }`}>
            ...
            <div className={`p-4 rounded-2xl text-xs md:text-sm leading-relaxed ${
              msg.role === 'user'
                ...
                : 'bg-[#0c101a] text-slate-200 rounded-tl-none border border-white/5'
            }`}>
        ...
        {loading && (
          ...
          >
            <div className="w-8 h-8 rounded-xl bg-[#0c101a] border border-white/5 flex items-center justify-center text-blue-400 shrink-0 shadow-inner">
              <Bot className="w-4 h-4 animate-bounce" />
            </div>
            <div className="p-4 rounded-2xl bg-[#0c101a] border border-white/5 rounded-tl-none flex items-center gap-2">
      ...
      {messages.length === 1 && !loading && (
        <div className="px-6 py-2.5 border-t border-white/5 bg-[#0c101a]/50">
          ...
          <div className="flex flex-wrap gap-2">
            {templates.map((t, i) => (
              <motion.button
                ...
                className="text-[11px] font-medium text-slate-300 hover:text-blue-400 border border-white/5 bg-[#0c101a] rounded-xl py-1.5 px-3 transition-colors cursor-pointer"
              >
                {t.text}
              </motion.button>
            ))}
      ...
      {/* Input Form Bar */}
      <form onSubmit={handleSend} className="px-4 py-3 md:px-6 md:py-4 border-t border-white/5 bg-[#0c101a] flex items-center gap-3">
        <input
          type="text"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder={selectedDocId ? "Ask a question about this file..." : "Type your message or prompt here..."}
          className="flex-1 bg-[#101623] hover:bg-[#131a29] focus:bg-[#131a29] border border-white/5 focus:border-blue-500/50 rounded-xl py-3 px-4 text-white placeholder-slate-500 text-xs md:text-sm outline-none transition-all"
        />
        <button
          type="submit"
          disabled={!input.trim() || loading}
          className="p-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl shadow-lg shadow-blue-600/25 transition-all active:scale-95 disabled:opacity-50 disabled:scale-100 cursor-pointer"
        >
