    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          ...
          className="self-start md:self-auto flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#3b82f6] hover:bg-[#2563eb] text-white font-semibold text-xs md:text-sm shadow-md shadow-blue-500/25 transition-all active:scale-95 cursor-pointer"
        >
      ...
      {/* 4 Metric Cards Row (Matching Light & Dark Reference Screenshots) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
        ...
        {/* Card 2: MONTHLY INCOME / DOCUMENTS (Blue Tint) */}
        <motion.div
          whileHover={{ y: -3 }}
          onClick={() => navigate('/documents')}
          className="p-5 rounded-2xl bg-[#eff6ff] dark:bg-[#081528] border border-[#bfdbfe] dark:border-[#0f294a] hover:border-blue-500/50 transition-all flex flex-col justify-between cursor-pointer group shadow-sm"
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-blue-800 dark:text-slate-400 uppercase tracking-wider">Documents Hub</span>
            <div className="w-9 h-9 rounded-xl bg-blue-100 dark:bg-[#0c2242] border border-blue-200 dark:border-[#1d4474] text-blue-600 dark:text-[#3b82f6] flex items-center justify-center group-hover:scale-105 transition-transform">
              <TrendingUp className="w-4.5 h-4.5" />
            </div>
          </div>
          <div>
            <span className="text-2xl md:text-3xl font-extrabold text-blue-600 dark:text-[#3b82f6]">{data?.metrics.totalDocs || 0}</span>
            <p className="text-xs text-blue-700/80 dark:text-slate-400 mt-1">This month</p>
          </div>
        </motion.div>

        {/* Card 3: MONTHLY EXPENSES / ACTIVE PROJECTS (Red Tint) */}
        <motion.div
          whileHover={{ y: -3 }}
          onClick={() => navigate('/tasks')}
          className="p-5 rounded-2xl bg-[#fef2f2] dark:bg-[#1a0a0c] border border-[#fecaca] dark:border-[#381518] hover:border-red-500/50 transition-all flex flex-col justify-between cursor-pointer group shadow-sm"
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-red-800 dark:text-slate-400 uppercase tracking-wider">Active Projects</span>
            <div className="w-9 h-9 rounded-xl bg-red-100 dark:bg-[#2d1215] border border-red-200 dark:border-[#521c21] text-red-600 dark:text-[#ef4444] flex items-center justify-center group-hover:scale-105 transition-transform">
              <ArrowDownRight className="w-4.5 h-4.5" />
            </div>
          </div>
          <div>
            <span className="text-2xl md:text-3xl font-extrabold text-red-600 dark:text-[#ef4444]">{data?.metrics.totalProjects || 0}</span>
            <p className="text-xs text-red-700/80 dark:text-slate-400 mt-1">This month</p>
          </div>
        </motion.div>

        {/* Card 4: MONTHLY SAVINGS / TOTAL TASKS (Sky/Cyan Tint) */}
        <motion.div
          whileHover={{ y: -3 }}
          onClick={() => navigate('/tasks')}
          className="p-5 rounded-2xl bg-[#f0f9ff] dark:bg-[#081724] border border-[#bae6fd] dark:border-[#0e2c45] hover:border-sky-500/50 transition-all flex flex-col justify-between cursor-pointer group shadow-sm"
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-[10px] font-bold text-sky-800 dark:text-slate-400 uppercase tracking-wider">Total Tasks</span>
            <div className="w-9 h-9 rounded-xl bg-sky-100 dark:bg-[#0c2438] border border-sky-200 dark:border-[#164163] text-sky-600 dark:text-[#38bdf8] flex items-center justify-center group-hover:scale-105 transition-transform">
              <CheckSquare className="w-4.5 h-4.5" />
            </div>
          </div>
          <div>
            <span className="text-2xl md:text-3xl font-extrabold text-sky-600 dark:text-[#38bdf8]">{data?.metrics.totalTasks || 0}</span>
            <p className="text-xs text-sky-700/80 dark:text-slate-400 mt-1">Completed milestones</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3 text-xs">
              <span className="flex items-center gap-1.5 text-blue-600 dark:text-[#3b82f6] font-medium">
                <span className="w-2.5 h-2.5 rounded-full bg-blue-600 dark:bg-[#3b82f6]" /> AI Usage
              </span>
              <span className="flex items-center gap-1.5 text-red-600 dark:text-[#ef4444] font-medium">
                <span className="w-2.5 h-2.5 rounded-full bg-red-600 dark:bg-[#ef4444]" /> Latency
              </span>
            </div>
          </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ color: 'var(--text-title, #0f172a)', fontWeight: 'bold' }}
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-[#0d0d0d] border border-slate-200 dark:border-[#1c1c1c] shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-bold text-amber-600 dark:text-[#f59e0b] bg-amber-500/10 px-2.5 py-1 rounded-lg">
              100% Active
            </span>
          </div>
