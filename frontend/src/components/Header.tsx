    ...
    <header className="h-16 border-b border-white/5 dark:border-white/5 border-black/5 px-4 md:px-6 flex items-center justify-between shrink-0 glass-panel relative z-20">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="md:hidden flex items-center gap-2 cursor-pointer font-bold" onClick={() => navigate('/dashboard')}>
          <span className="text-slate-100 font-heading text-sm">SkillForge AI</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <button
            ...
            className="p-2 rounded-lg bg-white/5 border border-white/5 hover:border-white/10 text-slate-400 hover:text-slate-200 transition-colors relative cursor-pointer"
          ...
          <AnimatePresence>
            {showNotifications && (
              <motion.div
                ...
                className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto rounded-xl glass-panel shadow-2xl p-4 border border-white/10 z-50 divide-y divide-white/5"
              >
                <div className="flex items-center justify-between pb-3">
                  <h3 className="text-xs font-bold font-heading text-slate-100 uppercase tracking-wider">
                    Notifications
                  </h3>
                  ...
                  notifications.map((notif) => (
                    <div
                      ...
                      className={`py-3 first:pt-3 transition-colors relative ${
                        !notif.read ? 'bg-indigo-500/5 -mx-4 px-4' : ''
                      }`}
                    >
                      {!notif.read && (
                        <button
        ...
        {user && (
          <div className="flex items-center gap-2.5">
            ...
            <motion.div
              ...
              className="w-8.5 h-8.5 rounded-full bg-slate-800 border border-white/5 flex items-center justify-center text-slate-300 shadow-inner"
            >
